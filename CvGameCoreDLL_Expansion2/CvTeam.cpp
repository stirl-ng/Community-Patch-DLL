...
{
	PRECONDITION(eIndex >= 0 && eIndex < MAX_TEAMS);
				...
				if (GET_PLAYER(ePlayer1).GetEspionage()->IsMyDiplomatVisitingThem(ePlayer2, true))
				{
					...
					if (iSpyIndex != -1)
					{
						...
						if(pNotifications)
						{
							...
							strNotification << pEspionage->m_aSpyList[iSpyIndex].GetSpyName();
...
void CvTeam::announceTechToPlayers(TechTypes eIndex, bool bPartial)
{
	...
	for(int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		if(kPlayer.isAlive() && kPlayer.getTeam() == GetID())
		{
			// Send to pipe for ALL players (no filtering - let Python handle it)
		...
		for(iI = 0; iI < MAX_CIV_TEAMS; iI++)
		{
			if(GET_TEAM((TeamTypes)iI).isAlive())
			{
				if(GET_TEAM((TeamTypes)iI).GetTeamTechs()->HasTech(eTech))
				{
					if(isHasMet((TeamTypes)iI))
					{
						PRECONDITION(iI != GetID(), "iI is not expected to be equal with GetID()");
						iCount++;
					}
				}
			}
